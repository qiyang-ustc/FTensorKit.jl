...
begin
	# OBC here
	irange,jrange,krange = 0.1:0.2:0.9,0.1:0.2:0.9,0.3:0.4:0.9
	vertices = [(i,j) for i = irange for j = jrange]
	edges_h = [((i,j),(i+step(irange),j)) for i=irange[1:end-1] for j=jrange]
	edges_v = [((i,j),(i,j+step(jrange))) for i=irange for j=jrange[1:end-1]]
	edges = vcat(edges_h,edges_v)
...
begin
	...
	nodebrush = nodestyle(:default, stroke("lightblue"),fill("lightblue"))
end;
